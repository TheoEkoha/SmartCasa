...
async function getClaimParams(options) {
  const cc = await (async () => {
    if (options.type === "erc1155") {
      const { getActiveClaimCondition: getActiveClaimCondition2 } = await import("./getActiveClaimCondition-MRQPG3X7.js");
    ...
    if (options.type === "erc721") {
      const { getActiveClaimCondition: getActiveClaimCondition2 } = await import("./getActiveClaimCondition-MXFKGH7I.js");
      return await getActiveClaimCondition2({
        contract: options.contract
      });
    }
    const { getActiveClaimCondition } = await import("./getActiveClaimCondition-VZGEZTMF.js");
  ...
  const allowlistProof = await (async () => {
    ...
    const { fetchProofsForClaimer } = await import("./fetch-proofs-for-claimers-UNSS4UZL.js");
  ...
  const currency = allowlistProof.currency && allowlistProof.currency !== ADDRESS_ZERO ? allowlistProof.currency : cc.currency;
...
//# sourceMappingURL=chunk-VJUULAY6.js.map
