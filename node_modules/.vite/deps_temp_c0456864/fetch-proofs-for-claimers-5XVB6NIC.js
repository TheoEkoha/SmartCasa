...
async function hashEntry(options) {
  ...
  const currencyDecimals = await (async () => {
    if (isNativeTokenAddress(currencyAddress) || currencyAddress === ADDRESS_ZERO) {
      return 18;
    }
    const [{ getContract }, { decimals: getDecimals }] = await Promise.all([
      import("./contract-CNTDBPXE.js"),
      import("./decimals-4LN7A37R.js")
    ]);
...
async function fetchProofsForClaimer(options) {
  ...
  const proof = tree.getHexProof(await hashEntry({
    entry,
    chain: contract.chain,
    client: contract.client,
    tokenDecimals: options.tokenDecimals
...
//# sourceMappingURL=fetch-proofs-for-claimers-5XVB6NIC.js.map
