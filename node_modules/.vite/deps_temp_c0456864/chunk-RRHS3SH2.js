import {
  allowance
} from "./chunk-CKG6ILT6.js";
import {
  parseEventLogs,
  prepareEvent
} from "./chunk-4XYGIIDG.js";
import {
  approve
} from "./chunk-WXDNSQP3.js";
import {
  prepareContractCall
...
function isNativeAAChain(chain) {
  return chain.id === 324 || chain.id === 300 || chain.id === 302 || chain.id === 11124;
}
...
async function sendBundlerRequest(args) {
  ...
  const entryPointVersion = getEntryPointVersion(options.entrypointAddress || ENTRYPOINT_ADDRESS_v0_6);
  const bundlerVersion = entryPointVersion === "v0.6" ? "v1" : "v2";
  const bundlerUrl = options.bundlerUrl ?? getDefaultBundlerUrl(options.chain, bundlerVersion);
...
async function getGasFees(args) {
  ...
  const entrypointVersion = getEntryPointVersion(bundlerOptions.entrypointAddress || ENTRYPOINT_ADDRESS_v0_6);
  const bundlerVersion = entrypointVersion === "v0.6" ? "v1" : "v2";
  const bundlerUrl = (bundlerOptions == null ? void 0 : bundlerOptions.bundlerUrl) ?? getDefaultBundlerUrl(chain, bundlerVersion);
...
async function populateUserOp_v0_7(args) {
  ...
  const factory = isDeployed ? void 0 : factoryContract.address;
  const factoryData = isDeployed ? "0x" : await encode(prepareCreateAccount({
    factoryContract,
    adminAddress,
...
async function populateUserOp_v0_6(args) {
  const { bundlerOptions, isDeployed, factoryContract, accountContract, adminAddress, sponsorGas, overrides, nonce, callData, maxFeePerGas, maxPriorityFeePerGas } = args;
  const { chain, client } = bundlerOptions;
  const initCode = isDeployed ? "0x" : await getAccountInitCode({
    factoryContract,
    adminAddress,
    accountSalt: overrides == null ? void 0 : overrides.accountSalt,
    createAccountOverride: overrides == null ? void 0 : overrides.createAccount
  ...
  const account = {
    ...
    async signMessage({ message }) {
      const [{ isContractDeployed: isContractDeployed2 }, { readContract: readContract2 }, { encodeAbiParameters: encodeAbiParameters2 }, { hashMessage }, { checkContractWalletSignature }] = await Promise.all([
        import("./is-contract-deployed-NCGAJZ7V.js"),
        import("./read-contract-UCICUJWZ.js"),
        import("./encodeAbiParameters-WMOJLZIJ.js"),
        import("./hashMessage-KSLX7WRV.js"),
        import("./checkContractWalletSignature-RXASKXDV.js")
      ]);
    ...
    async signTypedData(_typedData) {
      ...
      const [{ isContractDeployed: isContractDeployed2 }, { readContract: readContract2 }, { encodeAbiParameters: encodeAbiParameters2 }, { checkContractWalletSignedTypedData }] = await Promise.all([
        ...
        import("./read-contract-UCICUJWZ.js"),
        import("./encodeAbiParameters-WMOJLZIJ.js"),
        import("./checkContractWalletSignedTypedData-LPH4T76H.js")
      ]);
...
async function _deployAccount(args) {
  ...
  const [{ sendTransaction }, { prepareTransaction }] = await Promise.all([
    import("./send-transaction-45UXWPAZ.js"),
    import("./prepare-transaction-62RFKSIO.js")
  ]);
...
//# sourceMappingURL=chunk-RRHS3SH2.js.map
