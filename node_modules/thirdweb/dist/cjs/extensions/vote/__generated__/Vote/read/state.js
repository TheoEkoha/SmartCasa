"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.FN_SELECTOR = void 0;
exports.isStateSupported = isStateSupported;
exports.encodeStateParams = encodeStateParams;
exports.encodeState = encodeState;
exports.decodeStateResult = decodeStateResult;
exports.state = state;
const read_contract_js_1 = require("../../../../../transaction/read-contract.js");
const encodeAbiParameters_js_1 = require("../../../../../utils/abi/encodeAbiParameters.js");
const viem_1 = require("viem");
const detectExtension_js_1 = require("../../../../../utils/bytecode/detectExtension.js");
exports.FN_SELECTOR = "0x3e4f49e6";
const FN_INPUTS = [
    {
        type: "uint256",
        name: "proposalId",
    },
];
const FN_OUTPUTS = [
    {
        type: "uint8",
    },
];
/**
 * Checks if the `state` method is supported by the given contract.
 * @param availableSelectors An array of 4byte function selectors of the contract. You can get this in various ways, such as using "whatsabi" or if you have the ABI of the contract available you can use it to generate the selectors.
 * @returns A boolean indicating if the `state` method is supported.
 * @extension VOTE
 * @example
 * ```ts
 * import { isStateSupported } from "thirdweb/extensions/vote";
 *
 * const supported = isStateSupported(["0x..."]);
 * ```
 */
function isStateSupported(availableSelectors) {
    return (0, detectExtension_js_1.detectMethod)({
        availableSelectors,
        method: [exports.FN_SELECTOR, FN_INPUTS, FN_OUTPUTS],
    });
}
/**
 * Encodes the parameters for the "state" function.
 * @param options - The options for the state function.
 * @returns The encoded ABI parameters.
 * @extension VOTE
 * @example
 * ```ts
 * import { encodeStateParams } "thirdweb/extensions/vote";
 * const result = encodeStateParams({
 *  proposalId: ...,
 * });
 * ```
 */
function encodeStateParams(options) {
    return (0, encodeAbiParameters_js_1.encodeAbiParameters)(FN_INPUTS, [options.proposalId]);
}
/**
 * Encodes the "state" function into a Hex string with its parameters.
 * @param options - The options for the state function.
 * @returns The encoded hexadecimal string.
 * @extension VOTE
 * @example
 * ```ts
 * import { encodeState } "thirdweb/extensions/vote";
 * const result = encodeState({
 *  proposalId: ...,
 * });
 * ```
 */
function encodeState(options) {
    // we do a "manual" concat here to avoid the overhead of the "concatHex" function
    // we can do this because we know the specific formats of the values
    return (exports.FN_SELECTOR +
        encodeStateParams(options).slice(2));
}
/**
 * Decodes the result of the state function call.
 * @param result - The hexadecimal result to decode.
 * @returns The decoded result as per the FN_OUTPUTS definition.
 * @extension VOTE
 * @example
 * ```ts
 * import { decodeStateResult } from "thirdweb/extensions/vote";
 * const result = decodeStateResult("...");
 * ```
 */
function decodeStateResult(result) {
    return (0, viem_1.decodeAbiParameters)(FN_OUTPUTS, result)[0];
}
/**
 * Calls the "state" function on the contract.
 * @param options - The options for the state function.
 * @returns The parsed result of the function call.
 * @extension VOTE
 * @example
 * ```ts
 * import { state } from "thirdweb/extensions/vote";
 *
 * const result = await state({
 *  contract,
 *  proposalId: ...,
 * });
 *
 * ```
 */
async function state(options) {
    return (0, read_contract_js_1.readContract)({
        contract: options.contract,
        method: [exports.FN_SELECTOR, FN_INPUTS, FN_OUTPUTS],
        params: [options.proposalId],
    });
}
//# sourceMappingURL=state.js.map