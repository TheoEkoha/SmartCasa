"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.FN_SELECTOR = void 0;
exports.isGetPublicationSupported = isGetPublicationSupported;
exports.encodeGetPublicationParams = encodeGetPublicationParams;
exports.encodeGetPublication = encodeGetPublication;
exports.decodeGetPublicationResult = decodeGetPublicationResult;
exports.getPublication = getPublication;
const read_contract_js_1 = require("../../../../../transaction/read-contract.js");
const encodeAbiParameters_js_1 = require("../../../../../utils/abi/encodeAbiParameters.js");
const viem_1 = require("viem");
const detectExtension_js_1 = require("../../../../../utils/bytecode/detectExtension.js");
exports.FN_SELECTOR = "0x7385ebc9";
const FN_INPUTS = [
    {
        type: "uint256",
        name: "profileId",
    },
    {
        type: "uint256",
        name: "pubId",
    },
];
const FN_OUTPUTS = [
    {
        type: "tuple",
        components: [
            {
                type: "uint256",
                name: "pointedProfileId",
            },
            {
                type: "uint256",
                name: "pointedPubId",
            },
            {
                type: "string",
                name: "contentURI",
            },
            {
                type: "address",
                name: "referenceModule",
            },
            {
                type: "address",
                name: "__DEPRECATED__collectModule",
            },
            {
                type: "address",
                name: "__DEPRECATED__collectNFT",
            },
            {
                type: "uint8",
                name: "pubType",
            },
            {
                type: "uint256",
                name: "rootProfileId",
            },
            {
                type: "uint256",
                name: "rootPubId",
            },
        ],
    },
];
/**
 * Checks if the `getPublication` method is supported by the given contract.
 * @param availableSelectors An array of 4byte function selectors of the contract. You can get this in various ways, such as using "whatsabi" or if you have the ABI of the contract available you can use it to generate the selectors.
 * @returns A boolean indicating if the `getPublication` method is supported.
 * @extension LENS
 * @example
 * ```ts
 * import { isGetPublicationSupported } from "thirdweb/extensions/lens";
 *
 * const supported = isGetPublicationSupported(["0x..."]);
 * ```
 */
function isGetPublicationSupported(availableSelectors) {
    return (0, detectExtension_js_1.detectMethod)({
        availableSelectors,
        method: [exports.FN_SELECTOR, FN_INPUTS, FN_OUTPUTS],
    });
}
/**
 * Encodes the parameters for the "getPublication" function.
 * @param options - The options for the getPublication function.
 * @returns The encoded ABI parameters.
 * @extension LENS
 * @example
 * ```ts
 * import { encodeGetPublicationParams } "thirdweb/extensions/lens";
 * const result = encodeGetPublicationParams({
 *  profileId: ...,
 *  pubId: ...,
 * });
 * ```
 */
function encodeGetPublicationParams(options) {
    return (0, encodeAbiParameters_js_1.encodeAbiParameters)(FN_INPUTS, [options.profileId, options.pubId]);
}
/**
 * Encodes the "getPublication" function into a Hex string with its parameters.
 * @param options - The options for the getPublication function.
 * @returns The encoded hexadecimal string.
 * @extension LENS
 * @example
 * ```ts
 * import { encodeGetPublication } "thirdweb/extensions/lens";
 * const result = encodeGetPublication({
 *  profileId: ...,
 *  pubId: ...,
 * });
 * ```
 */
function encodeGetPublication(options) {
    // we do a "manual" concat here to avoid the overhead of the "concatHex" function
    // we can do this because we know the specific formats of the values
    return (exports.FN_SELECTOR +
        encodeGetPublicationParams(options).slice(2));
}
/**
 * Decodes the result of the getPublication function call.
 * @param result - The hexadecimal result to decode.
 * @returns The decoded result as per the FN_OUTPUTS definition.
 * @extension LENS
 * @example
 * ```ts
 * import { decodeGetPublicationResult } from "thirdweb/extensions/lens";
 * const result = decodeGetPublicationResult("...");
 * ```
 */
function decodeGetPublicationResult(result) {
    return (0, viem_1.decodeAbiParameters)(FN_OUTPUTS, result)[0];
}
/**
 * Calls the "getPublication" function on the contract.
 * @param options - The options for the getPublication function.
 * @returns The parsed result of the function call.
 * @extension LENS
 * @example
 * ```ts
 * import { getPublication } from "thirdweb/extensions/lens";
 *
 * const result = await getPublication({
 *  contract,
 *  profileId: ...,
 *  pubId: ...,
 * });
 *
 * ```
 */
async function getPublication(options) {
    return (0, read_contract_js_1.readContract)({
        contract: options.contract,
        method: [exports.FN_SELECTOR, FN_INPUTS, FN_OUTPUTS],
        params: [options.profileId, options.pubId],
    });
}
//# sourceMappingURL=getPublication.js.map